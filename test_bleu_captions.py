...
def validate_test(val_loader, encoder, decoder, criterion, maxSeqLen,
    ...
    with torch.no_grad():
        ...
        bleu1_avg = 0
        bleu4_avg = 0
        bleu4_sentence_avg = 0
        bleu1_sentence_avg = 0
        for i, (inputs, caps, allcaps) in enumerate(val_loader):
            ...
            bleu4_sentence = sentence_bleu(references, sampled_caption,weights=(0,0,0,1.0))#(0.25, 0.25, 0.25, 0.25))                
            bleu1_sentence = sentence_bleu(references, sampled_caption,weights=(1.0, 0, 0, 0))
            
            
            print('bleu4_sentence: ', bleu4_sentence)
            print('bleu1_sentence: ', bleu1_sentence)
            print('DBG: reference_caption: ', references)
            print('DBG: sampled_caption: ',sampled_caption)
            bleu4_sentence_avg+=bleu4_sentence
            bleu1_sentence_avg+=bleu1_sentence
        ...
        if calculate_bleu:
            ...
            #print('len(references)',len(references))
            #print('len(hypotheses)',len(hypotheses))
            bleu4 = corpus_bleu(references, hypotheses,weights=(0, 0, 0, 1.0))                
...
#            bleu1_avg+=bleu1
        ...
        if calculate_bleu:
            ...
            bleu4_avg = bleu4
            bleu1_avg = bleu1 
            bleu4_sentence_avg = bleu4_sentence_avg/count
            bleu1_sentence_avg = bleu1_sentence_avg/count

            print('VAL: bleu4_avg: ', bleu4_avg)
            print('VAL: bleu1_avg: ', bleu1_avg)
        
            print('VAL: bleu4_sentence_avg: ', bleu4_sentence_avg)
            print('VAL: bleu1_sentence_avg: ', bleu1_sentence_avg)
...
if __name__=='__main__':
    ...
    encoder = torch.load('saved_weights/lstmencoder_epoch6')
    decoder = torch.load('saved_weights/lstmdecoder_epoch6')  
