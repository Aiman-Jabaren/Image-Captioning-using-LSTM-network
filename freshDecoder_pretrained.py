...
class Decoder(nn.Module):
    def __init__(self, encoded_feature_dim, hidden_dim, depth, vocab_size, batch_size):
        ...
        self.target_vocab = pickle.load(open('data/vocab.pkl','rb'))
        
        self.embedding_dim = 50
        self.embed_map = weights(self.target_vocab)
#         self.embedding, self.num_embeddings, self.embedding_dim = emb_layer(weights(self.target_vocab), non_train=True) #input to include target 
        
        self.initial_fc = nn.Linear(encoded_feature_dim, self.embedding_dim)
        ...
        self.lstm = nn.LSTM(self.embedding_dim, hidden_dim, depth, batch_first=True)
...
#         embeds = self.embedding(sentence)
        embeds = embed(sentence, self.embed_map)
...
#         print('hidden shape: ', self.hidden[0].size())
#         print('hidden shape: ', self.hidden[1].size())
        lstmOut, self.hidden = self.lstm(features.unsqueeze(1), self.hidden)
        outputs.append(lstmOut)
        for i in range(embeds.shape[1] - 1):
            lstmOut, self.hidden = self.lstm(embeds[:,i,:].unsqueeze(1).to(self.device), self.hidden)
            outputs.append(lstmOut)
